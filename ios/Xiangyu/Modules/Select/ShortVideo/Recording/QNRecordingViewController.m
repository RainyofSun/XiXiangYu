...
@property (nonatomic, strong) QNVerticalButton *speedButton;
@property (nonatomic, strong) QNVerticalButton *faceUnityButton;
...
//    self.recordingProgress.clipsToBounds = YES;
  //qn_effect
    ...
    [self.recordingButton setImage:[UIImage imageNamed:@"Kaishipaishe"] forState:UIControlStateNormal];
    [self.recordingButton setImage:[UIImage imageNamed:@"Kaishipaishe"] forState:(UIControlStateSelected)];
    [self.recordingButton addTarget:self action:@selector(clickRecordingButton:) forControlEvents:(UIControlEventTouchUpInside)];
    self.recordingButton.layer.cornerRadius = 28;
...
//    [self.rateControl addSubview:self.rateIndicatorView];
    ...
    self.nextButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 160, 44)];
    self.nextButton.titleLabel.font = [UIFont systemFontOfSize:20];
  self.nextButton.hidden = YES;
    self.nextButton.layer.cornerRadius = 22.0;
    [self.nextButton setTitle:@"下一步" forState:(UIControlStateNormal)];
    [self.nextButton setBackgroundColor:ColorHex(@"#F92B5E")];
    ...
    [self.flashButton setImage:[UIImage imageNamed:@"buguang"] forState:(UIControlStateNormal)];
    [self.flashButton setImage:[UIImage imageNamed:@"buguang"] forState:(UIControlStateSelected)];
  ...
  [self.cameraButton setImage:[UIImage imageNamed:@"fanzhuan"] forState:(UIControlStateNormal)];
  ...
  [self.cameraButton addTarget:self action:@selector(clickCameraButton:) forControlEvents:(UIControlEventTouchUpInside)];
    ...
    self.musicButton.titleLabel.font = [UIFont systemFontOfSize:12];
    [self.musicButton setImage:[UIImage imageNamed:@"yinyue"] forState:(UIControlStateNormal)];
    [self.musicButton setTitle:@"  选择音乐" forState:(UIControlStateNormal)];
    [self.musicButton addTarget:self action:@selector(clickMusicButton:) forControlEvents:(UIControlEventTouchUpInside)];
    
  self.faceUnityButton = [[QNVerticalButton alloc] init];
  [self.faceUnityButton setImage:[UIImage imageNamed:@"meiyan"] forState:(UIControlStateNormal)];
  self.faceUnityButton.titleLabel.font = [UIFont systemFontOfSize:14];
  [self.faceUnityButton setTitle:@"美颜" forState:(UIControlStateNormal)];
  [self.faceUnityButton addTarget:self action:@selector(clickFaceUnityButton:) forControlEvents:(UIControlEventTouchUpInside)];
    ...
    [self.filterButton setImage:[UIImage imageNamed:@"lvjing"] forState:(UIControlStateNormal)];
  ...
  self.speedButton = [[QNVerticalButton alloc] init];
  self.speedButton.titleLabel.font = [UIFont systemFontOfSize:14];
  [self.speedButton setImage:[UIImage imageNamed:@"biansu"] forState:(UIControlStateNormal)];
  [self.speedButton setTitle:@"变速" forState:(UIControlStateNormal)];
  [self.speedButton addTarget:self action:@selector(clickSpeedButton:) forControlEvents:(UIControlEventTouchUpInside)];
  ...
  [superView addSubview:self.speedButton];
  ...
  [self.filterButton mas_makeConstraints:^(MASConstraintMaker *make) {
      make.right.equalTo(self.cameraButton);
  ...
  [self.faceUnityButton mas_makeConstraints:^(MASConstraintMaker *make) {
    make.right.equalTo(self.cameraButton);
    make.size.equalTo(self.cameraButton);
    make.top.equalTo(self.filterButton.mas_bottom).offset(8);
  }];
  
  [self.speedButton mas_makeConstraints:^(MASConstraintMaker *make) {
      make.right.equalTo(self.filterButton);
      make.size.equalTo(self.filterButton);
      make.top.equalTo(self.faceUnityButton.mas_bottom).offset(8);
  }];
  [self.flashButton mas_makeConstraints:^(MASConstraintMaker *make) {
    make.right.equalTo(self.speedButton);
    make.size.equalTo(self.speedButton);
    make.top.equalTo(self.speedButton.mas_bottom).offset(8);
  }];
  ...
  [superView addSubview:self.cycleView];
...
//    }];
  ...
  [self.cycleView mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [self.recordingButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(superView);
      make.centerY.equalTo(self.cycleView);
        make.size.equalTo(CGSizeMake(56, 56));
    }];
    
  [self.nextButton mas_makeConstraints:^(MASConstraintMaker *make) {
      make.center.equalTo(self.recordingButton);
      make.size.equalTo(CGSizeMake(160, 44));
...
//        make.centerX.mas_equalTo(self.recordingButton.centerX);
      make.centerX.mas_equalTo(self.recordingButton.center);
        make.bottom.mas_equalTo(self.recordingButton.mas_top).offset(-5);
    }];
...
- (void)setupRecorder {
    ...
    self.recorder.maxDuration = 60; // 设置最大录制时长为 15s
...
- (void)setupFaceUnityUI {
  ...
  self.faceUnityView.recorder = self.recorder;
...
-(void)sliderOnchange:(UISlider *)slider{
  
  [self.recorder setBeautify:slider.value];
}
...
- (void)clickBackButton {
    if ([self.recorder getFilesCount] > 0 || [self.recorder isRecording]) {
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"确定退出?" message:@"现在退出视频将不会保存" preferredStyle:(UIAlertControllerStyleAlert)];
...
//    if (0 == [self.recorder getFilesCount]) return;
  ...
  [self.cycleView drawProgress:0.0];
...
- (void)clickRecordingButton:(UIButton *)button {
    if (self.captureModeControl.selectedIndex == 1) {
        ...
        if (self.recorder.isRecording) {
          ...
          //XYToastText();
//          NSLog(@"===  %@",[NSString stringWithFormat:@"录制时间 %f",[self.recorder getFilesCount]]);
           
          
//          if ( self.currentRecordingTime  > self.recorder.minDuration) {
...
- (void)clickFlashButton:(UIButton *)button {
  if (AVCaptureDevicePositionFront == self.recorder.captureDevicePosition){
    [self.view showTip:@"需要启用背面摄像头才能启用补光"];
    return;
  }
...
- (void)clickSpeedButton:(UIButton *)button {
    self.filterPickerView.hidden = NO;
  self.rateControl.hidden = !self.rateControl.hidden;
...
//    [self.recordingProgress stopShining];
  ...
  if (self.nextButton.hidden) {
    [self.recordingButton alphaShowAnimation];
    [self.cycleView alphaShowAnimation];
    [self.durationLabel alphaShowAnimation];
  }else{
    [self.recordingButton alphaHideAnimation];
    [self.cycleView alphaHideAnimation];
    [self.durationLabel alphaHideAnimation];
  }
...
//    [self.recordingProgress setLastProgressToWidth:fileDuration / self.recorder.maxDuration * self.recordingProgress.frame.size.width];
  [self.cycleView drawProgress:totalDuration/60.0f ];
  self.currentRecordingTime = totalDuration;
