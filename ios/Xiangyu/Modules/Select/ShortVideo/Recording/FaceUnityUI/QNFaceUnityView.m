...
@property(nonatomic,assign)CGFloat reddenValue;
@end
...
-(void)initBut{
  ...
  self.whitenValue = 0.5;
  self.reddenValue = 0.5;
}
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
      [self initBut];
        ...
        self.slider.minimumTrackTintColor = ColorHex(XYTextColor_FE2D63);
        ...
        [self.noneButton setImage:[UIImage imageNamed:@"huifumoren"] forState:(UIControlStateNormal)];
      [self.noneButton setTitle:@"恢复默认" forState:UIControlStateNormal];
      self.noneButton.titleLabel.font = [UIFont systemFontOfSize:14];
...
//        }];
        ...
        [self.noneButton mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.size.equalTo(CGSizeMake(130, 34));
        }];
...
- (void)beautyView:(QNFaceUnityBeautyView *)beautyView didSelectedIndex:(NSInteger)selectedIndex {
    switch (selectedIndex) {
        ...
        [self.recorder setBeautifyModeOn:YES];
            self.slider.value = self.beautyValue;
            break;
        case 2:
        [self.recorder setBeautifyModeOn:YES];
        self.slider.value = self.whitenValue;
            break;
        case 3:
        [self.recorder setBeautifyModeOn:YES];
        self.slider.value = self.reddenValue;
            break;
        case 4:
...
- (void)updateLabel {
  ...
  [self.recorder setBeautify:self.beautyValue];
  [self.recorder setRedden:self.reddenValue];
  [self.recorder setWhiten:self.whitenValue];
