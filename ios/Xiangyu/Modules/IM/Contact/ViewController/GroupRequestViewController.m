...
- (void)viewDidLoad {
  ...
  [self.tableView mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [[V2TIMManager sharedInstance] getUsersInfo:@[loginUser] succ:^(NSArray<V2TIMUserFullInfo *> *infoList) {
        self.addMsgTextView.text = [NSString stringWithFormat:NSLocalizedString(@"%@ 申请加入群聊", nil), [[infoList firstObject] showName]]; // "%@ 申请加入群聊
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"添加", nil) style:UIBarButtonItemStylePlain target:self action:@selector(onSend)];
    self.title = NSLocalizedString(@"添加群组", nil);
}
...
{
    if (section == 1)
        return NSLocalizedString(@"请填写验证信息", nil); // @"填写验证信息";
    return nil;
}
...
- (void)onSend
{
    ...
    [[V2TIMManager sharedInstance] joinGroup:self.groupInfo.groupID msg:self.addMsgTextView.text succ:^{
        ...
        [self.view makeToast:NSLocalizedString(@"发送成功", nil)
                    duration:3.0
