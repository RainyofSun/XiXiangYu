...
@interface XYHeartRecordViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation XYHeartRecordViewController
...
-(void)getList{
  ...
  XYShowLoading;
  @weakify(self);
  XYProfessQueryXdList *api = [[XYProfessQueryXdList alloc]initWithOpType:@(self.heartType) page:self.page];
  api.filterCompletionHandler = ^(id  _Nullable data, XYError * _Nullable error) {
    ...
    XYQueryXdListObjModel *model = [XYQueryXdListObjModel yy_modelWithJSON:data];
    if (model.list.count) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
  XYHeartRecordTableViewCell *cell =[XYHeartRecordTableViewCell cellWithTableView:tableView indexPath:indexPath];
...
#pragma mark - 导航
-(void)newNav{
  self.gk_navTitle = @"我的心动记录";
  if (self.heartType == 1) {
    self.gk_navTitle = @"对我心动记录";
  }
}
/*
