...
@interface XYNearbyHomeScreenView ()
@property(nonatomic,strong)UIScrollView *scrollView;


@property(nonatomic,strong)XYScreeningGroupView *cityView;
@property(nonatomic,strong)XYScreeningSecectedGroupView *sexView;
...
@property(nonatomic,strong)XYFormattedArea *areaModel;



@property(nonatomic,strong)UIView *bottomView;
@end
@implementation XYNearbyHomeScreenView
...
-(void)setAreaModel:(XYFormattedArea *)model{
  _areaModel = model;
  self.proviceItem = [XYAddressItem new];
  self.proviceItem.name = model.provinceName;
  self.proviceItem.code = model.provinceCode;
  self.cityItem = [XYAddressItem new];
  self.cityItem.name = model.cityName;
  self.cityItem.code = model.cityCode;
  self.areaItem = [XYAddressItem new];
  self.areaItem.name = model.districtName;
  self.areaItem.code = model.code;
  [self setCityData];
}
-(void)newView{
...
//  }];
  ...
  [self.highSchoolView mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [self.sexView mas_makeConstraints:^(MASConstraintMaker *make) {
      make.leading.trailing.equalTo(self);
      make.top.equalTo(self.highSchoolView.mas_bottom).offset(AutoSize(10));
...
-(void)show{
  ...
  if (self.reqParams.dwellProvince) {
    self.proviceItem = [[XYAddressService sharedService] queryModelWithAdcode:self.reqParams.dwellProvince];
  }
  if (self.reqParams.dwellCity) {
    self.cityItem = [[XYAddressService sharedService] queryModelWithAdcode:self.reqParams.dwellCity];
  }
  if (self.reqParams.dwellArea) {
    self.areaItem = [[XYAddressService sharedService] queryModelWithAdcode:self.reqParams.dwellArea];
  }
  [self setCityData];
}
...
//  return _heightView;
//}
-(XYScreeningSecectedGroupView *)sexView{
  if (!_sexView) {
    _sexView = [[XYScreeningSecectedGroupView alloc] initWithFrame:CGRectMake(0, 0, AutoSize(300), AutoSize(40))];
    _sexView.title = @"性别";
    _sexView.dataSource = @[@{@"id":@(1),@"name":@"只看男性"},
    @{@"id":@(2),@"name":@"只看女性"},];
   // [_sexView reshEdu];
    @weakify(self);
    _sexView.selectedBlock = ^(NSDictionary * _Nonnull item) {
      @strongify(self);
      self.reqParams.sex = [item objectForKey:@"id"];
    };
  }
  return _sexView;
}
...
-(UIView *)bottomView{
  if (!_bottomView) {
    ...
    [confirmBtn handleControlEventWithBlock:^(id sender) {
      ...
      self.reqParams.dwellProvince = self.proviceItem.code;
      self.reqParams.dwellCity = self.cityItem.code;
      self.reqParams.dwellArea = self.areaItem.code;
...
//      self.reqParams.endHeight =  @((int)self.heightView.slider.selectedMaximum);
    ...
    [resetBtn handleControlEventWithBlock:^(id sender) {
      ...
      self.reqParams.sex = @(-10);
      [self.sexView resetData];
      [self setAreaModel:self.areaModel];
