...
#import "SVGAParser.h"
@interface XYReceiveGiftPopView ()
@property(nonatomic,strong)UIImageView *headerImage;
@property(nonatomic,strong)UILabel *titleLabel;
@property(nonatomic,strong)UILabel *contentLabel;
@property(nonatomic,strong)SVGAPlayer *player;
@property(nonatomic,strong)XYBlindGiftModel *itemModel;
@property(nonatomic,strong)UILabel *descLabel;
@property(nonatomic,strong)UIButton *closeBtn;
@property(nonatomic,strong)UIButton *chatButton;
@end
@implementation XYReceiveGiftPopView
...
-(void)getMyGift{
  ...
  XYGetGiftMesAPI *api = [[XYGetGiftMesAPI alloc] initWithOrderId:self.orderId];
  api.filterCompletionHandler = ^(NSDictionary * data, XYError * _Nullable error) {
    if (!error) {
      
      self.itemModel = [XYBlindGiftModel yy_modelWithJSON:data];
//      NSArray *list = data[@"list"];
//     NSArray *array = [NSArray yy_modelArrayWithClass:[XYBlindGiftModel class] json:list];
//      if (array.count) {
//        self.itemModel = array.firstObject;
//      }
...
-(void)setItemModel:(XYBlindGiftModel *)itemModel{
  _itemModel = itemModel;
  [self.headerImage sd_setImageWithURL:[NSURL URLWithString:itemModel.headPortrait]];
  
  self.titleLabel.text = [NSString stringWithFormat:@"你收到了%@个【%@】",itemModel.giftNum,itemModel.giftName];
  if ([itemModel.type integerValue] == 2) {
    self.contentLabel.text = @" ";
    self.descLabel.text = @"和TA互动超过3句即可获得现金奖励哦~";
  }else{
    self.contentLabel.text = @"礼物已转为相应数额乡币进入您的乡币账户 乡币可兑换精美好礼";
    self.descLabel.text = @" ";
    //
  }
  ...
  SVGAParser *parser = [[SVGAParser alloc] init];
      @weakify(self);
  [parser parseWithURL:[NSURL URLWithString:itemModel.animation] completionBlock:^(SVGAVideoEntity * _Nullable videoItem) {
    @strongify(self);
...
-(void)newView{
  ...
  self.contentLabel = [LSHControl createLabelFromFont:AdaptedFont(16) textColor:ColorHex(@"#CFCFCF") text:@""];
...
//
  ...
  [self.player mas_makeConstraints:^(MASConstraintMaker *make) {
    make.centerX.equalTo(self);
    make.top.equalTo(self.titleLabel.mas_bottom).offset(AutoSize(12));
    make.size.mas_equalTo(CGSizeMake(AutoSize(343), AutoSize(230)));
  }];
  self.descLabel = [LSHControl createLabelFromFont:AdaptedFont(14) textColor:ColorHex(@"#CFCFCF") text:@" "];//
