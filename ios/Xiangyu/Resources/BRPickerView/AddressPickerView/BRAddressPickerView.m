 ...
 *  @param selectValues             默认选中的值(传索引数组，如：@[@"", @"", @""])
...
- (NSArray <BRProvinceModel *>*)getProvinceModelArr:(NSArray *)dataSourceArr {
    ...
    for (NSDictionary *proviceDic in dataSourceArr) {
        ...
        NSArray *cityList = [proviceDic.allKeys containsObject:@"districts"] ? [proviceDic objectForKey:@"districts"] : [proviceDic objectForKey:@"districts"];
        ...
        for (NSDictionary *cityDic in cityList) {
            ...
            NSArray *areaList = [cityDic.allKeys containsObject:@"districts"] ? [cityDic objectForKey:@"districts"] : [cityDic objectForKey:@"districts"];
