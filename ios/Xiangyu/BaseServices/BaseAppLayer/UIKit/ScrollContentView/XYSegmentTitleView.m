...
#import "XYSegmentTitleView.h"

@interface XYSegmentTitleView ()<UIScrollViewDelegate>
...
@property (nonatomic, assign) XYIndicatorType indicatorType;

@property (nonatomic, strong) NSArray *titlesArr;

@end

@implementation XYSegmentTitleView
...
- (void)layoutSubviews
{
    ...
    if (_titleFont != _titleSelectFont) {
        for (int idx = 0; idx < self.titlesArr.count; idx++) {
            ...
            CGFloat itemBtnWidth = [XYSegmentTitleView getWidthWithString:self.titlesArr[idx] font:titleFont] + self.itemMargin;
    ...
    else
    {
        for (NSString *title in self.titlesArr) {
            CGFloat itemBtnWidth = [XYSegmentTitleView getWidthWithString:title font:titleFont] + self.itemMargin;
            totalBtnWidth += itemBtnWidth;
        }
    }
...
- (void)moveIndicatorView:(BOOL)animated
{
    ...
    CGFloat indicatorWidth = [XYSegmentTitleView getWidthWithString:self.titlesArr[self.selectIndex] font:titleFont];
    [UIView animateWithDuration:(animated?0.05:0) animations:^{
        switch (_indicatorType) {
            case XYIndicatorTypeDefault:
                ...
                break;
            case XYIndicatorTypeEqualTitle:
...
- (void)btnClick:(UIButton *)btn
{
    ...
    if (self.delegate&&[self.delegate respondsToSelector:@selector(XYSegmentTitleView:startIndex:endIndex:)]) {
        [self.delegate XYSegmentTitleView:self startIndex:self.selectIndex endIndex:index];
    }
...
{
    if (self.delegate&&[self.delegate respondsToSelector:@selector(XYSegmentTitleViewWillBeginDragging:)]) {
        [self.delegate XYSegmentTitleViewWillBeginDragging:self];
    }
}

- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    if (self.delegate&&[self.delegate respondsToSelector:@selector(XYSegmentTitleViewWillEndDragging:)]) {
        [self.delegate XYSegmentTitleViewWillEndDragging:self];
    }
}
