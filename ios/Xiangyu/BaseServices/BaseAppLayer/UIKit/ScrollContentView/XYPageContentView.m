...
#import "XYPageContentView.h"

#define IOS_VERSION ([[[UIDevice currentDevice] systemVersion] floatValue])
static NSString *collectionCellIdentifier = @"collectionCellIdentifier";

@interface XYPageContentView ()<UICollectionViewDelegate,UICollectionViewDataSource>
...
@implementation XYPageContentView
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(XYContentViewWillBeginDragging:)]) {
        [self.delegate XYContentViewWillBeginDragging:self];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(XYContentViewDidScroll:startIndex:endIndex:progress:)]) {
        [self.delegate XYContentViewDidScroll:self startIndex:startIndex endIndex:endIndex progress:progress];
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(XYContenViewDidEndDecelerating:startIndex:endIndex:)]) {
        [self.delegate XYContenViewDidEndDecelerating:self startIndex:startIndex endIndex:endIndex];
    }
}
...
{
    if (!decelerate) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(XYContenViewDidEndDragging:)]) {
            [self.delegate XYContenViewDidEndDragging:self];
        }
    }
}
